...
orange = drive up the planned step, red = the owned ramps cannot bring this wheel
within tolerance. The display stabilizer adopts a differing fresh plan only when it is
...
- A wheel can now be red **with** a planned step: driving up is still worth it, but
  the set cannot finish the job — the status line says so.
