...
| Status characteristic battery/temperature byte layout      | Bytes 2–3 (LE uint16 `rawMv`): battery% = `clamp(rawMv × 0.1 − 200, 0, 100)`. Byte 7: firmware tier (thresholds 32/48/64/80/96/112 → tiers 1–7), selecting the temperature formula — tier 1: `clamp(byte[0] / 16 + 25, −40, 80)`; tier 2+: `clamp(int16LE(bytes[0..1]) / 100, −40, 80)`. Re-confirmed by a second direct read of the same decompiled bytecode (#123) — was previously under-reported as undecoded beyond byte 7. |
...
| Device name prefix                                         | Devices identify as `"CARATI..."` (per #119)                                                                                                                                                                                                                                                                                                                                                                                     |
| Secondary scan UUID `669a0c20-0008-a7ba-e311-0685c0f7978a` | Present as a declared constant in the decompiled code, but no other reference to it was found — role (advertised scan filter vs. dead code) is **Unknown**, not yet checked against a physical device                                                                                                                                                                                                                            |
