...
| Sensor-mounting rotation (`"sensor_Placing"` pref)      | **Verified (#215)** as existing, **Unknown** for any specific installed box. The app's settings screen offers two mounting illustrations (`top_wideside_rv.webp` / `top_shortside_rv.webp` — the same box rotated 90°, wide-edge-forward vs short-edge-forward); `LO0/e;->k()` implements the non-default option as a 90° axis swap with sign changes. This is an installation fact the box itself never transmits, so no packet inspection recovers it for a given install — Libell has no equivalent setting and assumes the app's default mounting.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
